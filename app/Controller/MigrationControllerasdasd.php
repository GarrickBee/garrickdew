...
class MigrationController extends AppController
{
	...
	public function submit()
	{
		...
		for ($row = 2; $row <= $highest_row; $row++)
		{
			...
			$member_type = explode(" ",$row_data[3])[0];
			$member_no   = intval(explode(" ",$row_data[3])[1]);

			// Check duplicate Member ID in excel file
			// Assuming Member No is unique as Member ID
			// $check_type = in_array($member_type, array_column($members_data,'type'));
			// $check_no   = in_array($member_no,array_column($members_data,'no'));
		...
		$current_member_data = $members_data;
		...
		foreach ($transaction_data as $transaction_key => $transaction_value)
		{
			...
			unset($transaction_value['type']);
			unset($transaction_value['no']);
			$data[] = array(
				'member' => $members_data,
	...
	{
		// Check user that has been registered in current database
		...
		{
			$check_type = in_array($member_value['type'],array_column($current_member_data,'type'));
			$check_no   = in_array($member_value['no'],array_column($current_member_data,'no'));
			if ( $check_type && $check_no )
			{
